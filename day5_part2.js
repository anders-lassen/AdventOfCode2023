...
//     var locations = []
    ...
    //     part 2: seeds consist of pairs of ranges
    ...
    for (let i = 0; i < data.seeds.length; i += 2) {
        const seed_range_start = data.seeds[i];
        const length = data.seeds[i + 1];

        console.log("seed_range_start", seed_range_start, "length", length)

        // for (let j = 0; j < length; j++) {
        //     let seed = seed_range_start + j;
            let seed = seed_range_start;
        ...
        //     ret_seed = findSeed(seed, "seed")
            ...
            if (!lowest_location || ret_seed < lowest_location)
                lowest_location = ret_seed
            // locations.push(ret_seed)
            //     });
        // }
    }

    console.log("lowest_location", lowest_location)
